...
- [x] **Step 1: 把 settings 测试改为跨模块一致性断言**
...
- Task 1 red: `KeyError: 'args'`；green: `tests/test_gui_bridge.py` 4 passed；提交 `f826714`。
- Task 2 red: `(1266, 1013) != (1280, 800)`；green: GUI app/settings 15 passed；提交 `614f99e`。
- Task 3 red: README 缺少“不可交互的白色窗口”；green: 文档测试 3 passed；提交 `7538d0d`。
- 全量 Python：pytest 77 passed，ruff check/format、mypy、py_compile 均通过。
- 前端：Node 15 passed，Svelte 0 errors/0 warnings，Vite 8.1.2 build 成功。PowerShell 必须使用 `npm.cmd`；检查与构建需串行，避免 `.vite-temp` 写入竞争。
- 真实 GUI：用户确认“正常且无白窗”；Chrome 根进程检测为 `Headless=True`、`Offscreen=True`。
