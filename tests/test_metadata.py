...
def test_embed_metadata_dispatches_to_flac(monkeypatch):
    calls = []
    monkeypatch.setattr(metadata, "embed_flac_tags", lambda *args, **kwargs: calls.append(("flac", args, kwargs)))
    monkeypatch.setattr(metadata, "embed_id3_tags", lambda *args, **kwargs: calls.append(("id3", args, kwargs)))
