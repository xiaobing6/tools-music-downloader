...
- 支持网易云音乐、咪咕音乐、酷狗音乐、酷我音乐、YouTube Music、Tidal、Qobuz、Deezer、Spotify、QQ音乐、喜马拉雅、JOOX、Apple Music 共 13 个音乐源。
...
- 下载响应会在落盘前保守排除明显的 HTML、JSON、XML 错误页，未知 MIME 的正常二进制音频仍可下载。
- GUI 每次启动都使用默认参数，不保存来源、搜索类型、数量、音质等用户选择。
- GUI 启动时显示品牌启动页和阶段化进度条；启动细节写入运行日志，不直接显示在启动页上。

## 支持的音源

CLI 参数、配置和上游请求仍使用稳定的音源 ID；GUI、CLI 表格、列表、提示和日志显示对应名称。CLI 参数仍使用音源 ID，例如 `--source netease`。
...
酷狗音乐可用于普通搜索；上游不保证酷狗专辑搜索可用。若某个音源的上游能力临时变化，工具会保留原始音源 ID，不会把未知音源渲染为空白。
...
搜索结果采用紧凑音乐库布局：每行保持 `60px`，歌曲/歌手、专辑和来源/状态使用内容加权的弹性列，时长使用兼容长时长的 `64px` 右对齐列；音源显示统一目录中的展示名称，未知时长显示为破折号。选中项使用浅蓝底和左侧品牌色标记，总数与已选数量集中显示在结果区标题下方；下载按钮文字保持稳定为“下载选中”，避免选择数量变化挤动操作区。搜索完成后封面逐张加载，不阻塞结果返回；单张封面解析失败时继续显示默认图标。

搜索关键词为空或搜索失败时，搜索框下方会显示可见提示，详细异常仍写入运行日志；搜索开始、完成、无结果和结果数量变化会通过适度的辅助技术状态播报反馈。
...
| `-s / --source` | 音乐源 ID，完整 ID 与展示名见“支持的音源” | `netease` |
